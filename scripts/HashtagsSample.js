...
var { _scroll_id, hits } = await client.search({
    // index: 'sample_twint',
    // index: 'twintsampletweets',
    // index: 'twint_politiki_tweets',
    index: 'twint_sample_tweets',
    scroll: '1000s',
    body: {
        query: {
            regexp: {
                // "text": ".*#[a-zA-Z0-9ćčđšžĆČĐŠŽ_]+ .*"
                hashtags: ".+",
                //     value: ".*\#Slovenija",
...
while(hits && hits.hits.length) {
    ...
    hits.hits.forEach(hits => {
        console.log(hits);
        // if (hits._source.username === 'javedi_si') return;
        if (new Date(hits._source.pub_date) <= new Date('2019-10-01')) return // Filter out
        const hashtag = hits._source.hashtags !== '' ? hits._source.hashtags : null;
...
fs.writeFile('hashtags_sample.txt', JSON.stringify(hashtags), function (err) {
